...
(function() {
   ...
   * modified by leon
   * _map {Map} 地图的一个实例。
   * _markers {Array<Marker>} 要聚合的标记数组
   * _girdSize {Number} 聚合计算时网格的像素大小，默认60
   * _maxZoom {Number} 最大的聚合级别，大于该级别就不进行相应的聚合
   *                   (这里我们设14,但应用已设<=14展示汇总,实际就不会发生聚合了）
   * _minClusterSize {Number} 最小的聚合数量，小于该数量的不能成为一个聚合，默认为2
   *                   (这里我们设20，其实没意义了，上一参数已控制了聚合）
   * _isAverangeCenter {Boolean} 聚合点的落脚位置是否是所有聚合在内点的平均值，默认为否，落脚在聚合内的第一个点
   * _styles {Array<IconStyle>} 自定义聚合后的图标风格，请参考TextIconOverlay类
   *
   * 添加 _boundMarkers 保存当前可视范围的marker
   *
   */
  ...
  // added by leon
  e.prototype._getBoundsMarker= function() {
    return this._boundMarkers
  }
  ...
  g.prototype.addMarker = function(k) {
    ...
    // 下面这段在addMarker() 方法内不停的去进行dom操作,引起卡顿的罪魁祸首 --- by leon
    // 注释掉，使用后面添加的 render 函数完成。(好像改善不大！)
