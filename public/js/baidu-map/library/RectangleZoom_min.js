...
(function() {
  ...
  c.prototype._updateStyle = function() {
    if (this._fDiv) {
      ...
      var m = this._fDiv.style, n = this._opts.opacity
      m.opacity = n
      m.MozOpacity = n
      m.KhtmlOpacity = n
  ...
  c.prototype.open = function() {
    ...
    var r = function(A) {
      ...
      var t = y.getSouthWest(), B = y.getNorthEast(), G = new BMap.Point(B.lng, t.lat),
        H = new BMap.Point(t.lng, B.lat), D = new BMap.Polygon([t, H, B, G])
      D.setStrokeWeight(2)
      ...
      p.addOverlay(D)
      new g({
        duration: 240, fps: 20, delay: 500, render: function(K) {
          var J = 0.3 * (1 - K)
  ...
  function g(p) {
    var m = {
      duration: 1000, fps: 30, delay: 0, transition: l.linear, onStop: function() {
      }
    }
    if (p) {
      for (var n in p) {
